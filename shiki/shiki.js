...
(() => {
  ...
  class Particle {
    ...
    isInTheAir() {
      ...
      const horizontalInView =
        this.x + this.width >= 0 && this.x <= windowWidth;
    ...
    moveFall() {
      if (this.isInTheAir()) {
        ...
        this.x = rand(
          this.width,
          Math.max(this.width, windowWidth - this.width),
        );
      }
    }
    ...
    moveFloat() {
      ...
      if (this.y < scroll - windowHeight * 0.3)
        this.y = scroll + windowHeight * 0.7;
      if (this.y > scroll + windowHeight * 1.3)
        this.y = scroll + windowHeight * 0.3;
    }
  ...
  const createParticle = (season, config, id) => {
    ...
    const cssVariant =
      config.cssVariants[rand(0, config.cssVariants.length - 1)];
    const animVariant =
      config.animVariants[rand(0, config.animVariants.length - 1)];
